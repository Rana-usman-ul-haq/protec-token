...
/*
Twitter/X: https://x.com/protecerc?s=21
...
contract Protec is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"Protec";
    string private constant _symbol = unicode"PRTC";
    uint256 public _taxSwapThreshold= 50000 * 10**_decimals;
    uint256 public _prtcMaxWalletSize = 2000000 * 10 ** decimals();
    ...
    constructor () {
      ...
      //  IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xD99D1c33F9fC3444f8101754aBC46c52416550D1); //testnet router
        ...
        _isExcludedFromFee[_prtcTreasuryWallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            ...
            if(to != uniswapV2Pair){
               require(balanceOf(to) + amount <= _prtcMaxWalletSize, "Alert: keep lower than Protec whale limits");
            }

            if(_prtcTaxOnBuy > 0) {
            if (from == uniswapV2Pair && to != address(uniswapV2Router)) {
                taxAmount = amount.mul(_prtcTaxOnBuy).div(100);
            }
            }

            if(_prtcTaxOnSell > 0) {
            if(to == uniswapV2Pair){
                taxAmount = amount.mul(_prtcTaxOnSell).div(100);
            }
            }
    ...
    function sendETHToFee(uint256 amount) private {
       (bool callSuccess, ) = payable(_prtcTreasuryWallet).call{value: amount}("");
